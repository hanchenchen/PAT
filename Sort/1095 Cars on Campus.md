...
2. 放弃`char[]`好写多了
   ...
   int charNum[maxS]={0};
   int main(){
       cin>>n>>k;int hh,mm,ss;char s[10];char sin[10]="in";
       for(int i=0;i<n;i++){
           scanf("%s",stu[i].id);
           scanf("%d:%d:%d",&hh,&mm,&ss);
           stu[i].times=getSecond(hh,mm,ss);
           scanf("%s",s);
           if(strcmp(s,sin)==0)stu[i].getIn=1;
       }
       ...
       vector<string> longestCar;int longestMunite=0;
       for(int i=0;i<n;i++){
           if(stu[i].getIn!=1)continue;
           int j=i+1;if(j>=n)break;
           if(strcmp(stu[i].id,stu[j].id)!=0||stu[j].getIn!=-1)continue;
           charNum[stu[i].times]++;
           charNum[stu[j].times]--;
           time_of_car[stu[i].id]+=stu[j].times-stu[i].times;
           if(longestMunite<time_of_car[stu[i].id]){
               longestMunite=time_of_car[stu[i].id];
               longestCar.clear();
               longestCar.push_back(stu[i].id);
...
3. ..可以直接使用`map<string,int>``vector<string>`放`char[]`，约等于`string`
